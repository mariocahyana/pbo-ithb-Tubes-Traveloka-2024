...
public Transaksi getTransactionByID(int transaksiID) {
    Transaksi transaksi = null;
    String query = "SELECT * FROM transaksi WHERE transaksiID = ?";
    try (Connection conn = dbHandler.getConnection();
         PreparedStatement stmt = conn.prepareStatement(query)) {
        ...
        if (rs.next()) {
            // Create a new Transaksi object and populate it with the retrieved data
            transaksi = new Transaksi();
            transaksi.setTransaksiID(rs.getInt("transaksiID"));
            // Assuming you have the proper setter methods and Transaksi class
            User user = new User();
            user.setUserID(rs.getInt("userID"));
            transaksi.setUser(user);
            transaksi.setPrice(rs.getDouble("price_transaction"));
            Flight flight = new Flight();
            flight.setFlightID(rs.getInt("flightID"));
            transaksi.setFlight(flight);
            transaksi.setNik(rs.getInt("nik"));
            transaksi.setSeat(rs.getString("seat_number"));
    ...
    return transaksi; // Returns null if no transaction is found
}
